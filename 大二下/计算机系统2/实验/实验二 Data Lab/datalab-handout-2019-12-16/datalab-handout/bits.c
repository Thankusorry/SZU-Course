 ...
 *   Rating: 3
 */
 // 先判断上四位等于 0x3 !(((x>>4)^0x3)^0x3)
int isAsciiDigit(int x) {
  return !( ((x&0xf0)^0x30) | !(x&0xA)^0xA | (x>>8)^0 | !(x&0xc)^0xc);
}
// 出现错误： 想得到某些为应该&所有位，而不是对应为1的位 ，比如 0x3 应该是 0xf,部分为就可以是 0xA
/*
...
!(x>y)
同号判断  !((x^y)>>31) : 同号返回1
同号：比较X-y<=0： !!((x+(~y+1))>>31) 负数1 返回1 op5
异号： 比较符号 x>0 y<0 return 0; x<0 y>0 return 1
 !(y>>31)&(x<<31) 两种情况都可以表示
*/
 ...
 *   Rating: 4
 */
 // 0001 ^ 0000 = 0001 特殊情况 10000000 也是 x==~x+1
// tmd 进位问题也要考虑
 ...
 */
 ...
 /*
 符号位怎么处理: 10xxxxx 可以表示 16+8+4+2+1+1=32
 但是 -1(0111111111)   只需要1位： 他的逻辑是什么：符号位！
 0111xxxx=16+8+4+2+0+1+1=30+1+1
 */
