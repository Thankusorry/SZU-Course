...
public class Client {
    ...
    private void init(){
        ...
        game_Button.addActionListener(new ActionListener()  {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    if (receiver == null || receiver.equals("聊天大厅")) {
                        JOptionPane.showMessageDialog(null, "请先选择对战用户！");
                        return;
                    }
                    // 发送邀请游戏指令
...
//                            else if(parts[0].equals("4321")) {
//                                String roomId = parts[1];
//                                String playerNum = parts[2];
//                                connectToGameServer(roomId);
//                                SwingUtilities.invokeLater(() -> {
//                                    JOptionPane.showMessageDialog(frame, "您是本局游戏的玩家" + playerNum);
//                                });
//                            }
                            // 在消息处理循环中添加以下case
                            else if (parts[0].equals("4321")) { // 开始游戏
                                currentRoomId = parts[1];
                                playerNumber = Integer.parseInt(parts[2]);
                                initGameUI();
                                ...
                                SwingUtilities.invokeLater(() -> {
                                    ...
                                    if(myTurn) JOptionPane.showMessageDialog(gameFrame, "轮到您下棋！");
                                    else{
                                        JOptionPane.showMessageDialog(gameFrame, "下一轮才到你！");
                                    }
                                });
                            }
...
//    }
    ...
    private void initGameUI() {
        if(gameFrame != null && gameFrame.isVisible()) return;
        gameFrame = new JFrame("五子棋对战");
        ...
        boardPanel.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                System.out.println("MyTurn="+myTurn);
                if (!myTurn) {
                    Toolkit.getDefaultToolkit().beep(); // 添加提示音
                    return;
                }
    ...
    private void sendWinMessage(int player) {
        String winMsg = "WIN:Player " + player + " wins!";
        byte[] data = winMsg.getBytes();
        try {
            DatagramPacket packet = new DatagramPacket(data, data.length,
                    InetAddress.getByName("127.0.0.1"), gamePort);
    ...
    // 在Client类中添加这个方法
    private void drawChessPiece(Graphics g, int x, int y, Color color) {
        int px = 40 + x * 40 - 15;
        int py = 40 + y * 40 - 15;
